    ...
    def dfs_utility(self, nodes, visited_nodes, colors, parity, level):
        ...
        if nodes == 1:
            ...
            if len(self.graph[nodes]) == 1 and parity % 2 == 0:
                if parity == 1:  # TODO changed q to parity
                    colors[nodes] = 1
                else:
                    colors[nodes] = -1
                    count += 1
...
class UnionFind():
    ...
    def answer(self, extra, p, q):
        ...
        for q in range(p):  # TODO changed n to p
...
def factors(n):
    return set(reduce(list.__add__, ([i, n // i] for i in range(1, int(n ** 0.5) + 1) if n % i == 0), [1]))
...
class SortedList:
    ...
    def _fen_findkth(self, k):
        """Return a pair of (the largest `idx` such that
        `sum(_fen_tree[:idx]) <= k`, `k - sum(_fen_tree[:idx])`)."""
